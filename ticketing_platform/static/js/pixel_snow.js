/**
 * PixelSnow — Vanilla JS port of the React Bits PixelSnow component
 * Uses Three.js via ES module import (CDN). No build step required.
 *
 * Usage:
 *   import { mountPixelSnow } from '/static/js/pixel_snow.js';
 *   const cleanup = await mountPixelSnow(document.getElementById('snow-container'), { ...opts });
 */
...
/**
 * Mount PixelSnow into a container element.
 * @param {HTMLElement} container
 * @param {object} opts
 * @returns {function} cleanup — call to destroy the renderer
 */
export async function mountPixelSnow(container, opts = {}) {
  const {
    color = '#ffffff',
    flakeSize = 0.01,
    minFlakeSize = 1.25,
    ...
    gamma = 0.4545,
    density = 0.3,
    variant = 'square',   // 'square' | 'round' | 'snowflake'
    direction = 125,
  } = opts;
  ...
  const colorVec = new Vector3(threeColor.r, threeColor.g, threeColor.b);
